...
DEPTH_RANGE_f = [-2.0,-0.3]#[-1.0,-0.3] # -1.0,-0.5
...
class PulseCamProcessorTF(threading.Thread):
	...
	def offset_to_a1_f(self, offset):
		o = np.array([(offset/10000)**k for k in range(len(self.cfgf[0]['a1_o']))])
		a1 = np.sum(o*self.cfgf[0]['a1_o'])
		return a1
	...
	def a1_to_offset_f(self, a1):
		...
		coeff = np.flipud(copy.deepcopy(self.cfgf[0]['a1_o']))
		coeff[-1] -= a1
		p = np.roots(coeff)
...
class Display(threading.Thread):
	...
	def iccv_output(self):	
		...
		Z_f = self.cfgf[0]['a1']
		flag = np.where(\
				np.abs(np.abs(self.results['Zf']) - Z_f) > 0.5*Z_f
			)
