...
class PulseCamProcessor(threading.Thread, training_pyConfLensFlowNetFast):
	...
	def __init__(self, cfg):
		...
		self.offset_labels = [k*1000 for k in range(55,20,-1)]
		self.inte = 0
		self.prev_err = 0
		...
		# collecting data
		self.I_cache = np.zeros(self.resolution[0]+(2,), dtype=np.float32)
		self.I_total = np.empty(
			(0,)+self.resolution[0]+(2,)
		)
			...
			if chr(c) == '\n':
				...
				while(in_key == '\n' or in_key == ''):
					...
					if in_key != '\n' and in_key != '':
						...
						in_key = input(
							"ENTER THE NUMBER (mm) ON THE TRANSLATION STAGE PANEL!\n"
						)
						if in_key != '\n' and in_key != '':
							if ending_key == 'q':
								break
				...
				# for offset in self.offset_labels:
				# 	self.offset = offset
				# 	self.cfg[0]['a1'] = 1/(1e-4*self.offset-1.9)
				# 	DEPTH_RANGE = [-self.cfg[0]['a1']-0.1,-self.cfg[0]['a1']+0.1]
				# 	KEY_RANGE['Z'] = DEPTH_RANGE
				# 	self.control_lens()
				# 	for waste_idx in range(30):
				# 		self.regular_output()

				self.I_cache = I_cache

				I_tmp = np.empty((1,)+self.resolution[0]+(2,))
