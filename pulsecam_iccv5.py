...
DEPTH_RANGE_f = [-3.0,-0.6]#[-1.0,-0.3] # -1.0,-0.5
...
class PulseCamProcessorTF(threading.Thread):
	...
	def build_graph(self):
		with self.graph.as_default():
			...
			for i in range(len(self.cfg)):
				...
				a1f_r = a1f + ra1_1[0] * r + ra1_2[0] * (r**2)
	...
	def offset_to_a1(self, offset):
		print(1/(self.cfg[0]['b0']*offset+self.cfg[0]['b1']))
		return 1/(self.cfg[0]['b0']*offset+self.cfg[0]['b1'])

	def a1_to_offset(self, a1):
		if a1 == 0:
			pdb.set_trace()
		print(int((1/a1 - self.cfg[0]['b1'])/self.cfg[0]['b0']))
...
class Display(threading.Thread):
	...
	def iccv_output(self):	
		...
		Z_f = self.cfgf[0]['a1']
		flag = np.where(\
				np.abs(np.abs(self.results['Zf']) - Z_f) > 0.3*Z_f
			)
		self.results['Zf'][flag] = 0
		self.results['conf'][flag] = 0

		# flag = np.where(\
		# 		np.abs(np.abs(self.results['Zf']) - Z_f) < 0.01*Z_f
		# 	)

		flag = np.where(\
				self.results['conf']<0.9
			)
...
def multithreading_test():
	...
	cfg_file = "./opt_results/pyConfLensFlowNetFast_iccv1/"+\
		"final.pickle"
	with open(cfg_file,'rb') as f:
