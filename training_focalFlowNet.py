...
PADDING = 'VALID'
class training_focalFlowNet(focalFlowNet):
	...
	def build_graph(self):
		with self.graph.as_default():
			...
			I_init = np.zeros(
				self.resolution+self.cfg['ft'].shape, 
				dtype = np.float32
			)
			Z_init = np.zeros(
				self.resolution,
			...
			Sigma_pix = Sigma / self.cfg['pixSize']
			...
			if 'dLdfxx' in self.cfg['der_var'][self.cur_err_idx]:
			...
			for ef in self.cfg['err_func']:
				...
				dLdft = dLdft - tf.reverse(dLdft, dims = [True])
				...
				dLdSigma, dLdmu_s, dLdf, dLdZ_0 = tf.gradients(
					self.vars['loss_func'][ef], [Sigma, mu_s, f, Z_0]
				)
			...
			for ef in self.cfg['err_func']:
				...
				for key in self.cfg['der_var'][self.cur_err_idx]:
					self.output_grads[ef][key] = self.der[key][ef]
				
			self.output_vars = {} # for variable updating
			for key in self.cfg['der_var'][self.cur_err_idx]:
				self.output_vars[key] = self.vars[self.var_dict[key]]
			
			#### put the used derivative into a list, depending on the cfg
			grads_and_vars = [
				der_pair[key] for key in self.cfg['der_var'][self.cur_err_idx]
			]
			self.grads_and_vars = grads_and_vars

			#### optimize using AdagradOptimizer
			opt = tf.train.AdagradOptimizer(learning_rate=self.cfg['learn_rate'])
