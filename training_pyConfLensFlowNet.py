...
PADDING = 'VALID'
class training_pyConfLensFlowNet(training_focalFlowNet):
	...
	def sig_err_all(self):
		...
		# will do the sum
		delta = 0.05
		k = self.resolution[0][0] * self.resolution[0][1] * len(self.cfg)
	...
	def sparsification_err(self):
		...
		k = self.resolution[0][0] * self.resolution[0][1]
	...
	def one_round_loss(self, I, I_lap, Loc):
		for i in range(len(I)):
			# input images
			Z_gt = Loc[i,2,int((Loc.shape[2]-1)/2)]
			Z_map_gt = np.ones(I[i][0].shape[0:2]) * Z_gt
			self.input_images(I[i], I_lap[i], Z_map_gt)

			# Update the loss
			self.update_loss()
		return self.print_loss()

	def one_step_training_force(self, I, I_lap, Loc, step, min_step, temperature = 0.0):
		...
		# self.visual_heatmap(I, I_lap, Loc, conf_thre=-np.inf)
		...
		while(step > min_step):
			...
			# run one round
			new_loss = self.one_round_loss(I, I_lap, Loc)
			thre = np.exp(
	...
	def visual_heatmap(self, I, I_lap, Loc, conf_thre=0.):
		...
		Z_map_gt = np.ones(I[0][0].shape[0:2]) * Z_gt
		self.input_images(I[0], I_lap[0], Z_map_gt)
		...
		for i in range(len(I)):
			...
			for j in range(len(self.cfg)):
				...
				Z_map_gt = np.ones(I[i][j].shape[0:2]) * Z_gt
	...
	def visual_heatmap_percent(self, I, I_lap, Loc, per_thre=0.):
		# input images
		Z_gt = Loc[0,2,int((Loc.shape[2]-1)/2)]
		Z_map_gt = np.ones(I[0][0].shape[0:2]) * Z_gt
		self.input_images(I[0], I_lap[0], Z_map_gt)
	...
	def visual_err_conf_map(self, I, I_lap, Loc, log=True):
		# input images
		Z_gt = Loc[0,2,int((Loc.shape[2]-1)/2)]
		Z_map_gt = np.ones(I[0][0].shape[0:2]) * Z_gt
		self.input_images(I[0], I_lap[0], Z_map_gt)
	...
	def sparsification_map(self, I, I_lap, Loc):
		# input images
		Z_gt = Loc[0,2,int((Loc.shape[2]-1)/2)]
		Z_map_gt = np.ones(I[0][0].shape[0:2]) * Z_gt
		self.input_images(I[0], I_lap[0], Z_map_gt)
	...
	def AUC_map(self, I, I_lap, Loc):
		...
		# save the data
		lpickle = len(glob.glob('./test_results/pyConfLensFlowNet/*.pickle'))
		fileName = os.path.join(\
			'./test_results/pyConfLensFlowNet/'+str(lpickle)+".pickle"
		)
		pdb.set_trace()
