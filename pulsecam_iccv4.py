...
DEPTH_RANGE_f = [-1.0,-0.3]#[-1.0,-0.3] # -1.0,-0.5
...
class PulseCamProcessorTF(threading.Thread):
	...
	def __init__(self, cfg):
		...
		# the port, always control offset
		self.offset_range = [36710,56535]
		self.offset = 50000
	...
	def build_graph(self):
		with self.graph.as_default():
			...
			for i in range(len(self.cfg)):
				...
				# compute a0
				a0_o.append(tf.constant(self.cfg[i]['a0_o'], dtype = tf.float32))
				a1_o.append(tf.constant(self.cfg[0]['a1_o'], dtype = tf.float32))
				...
				u_4.append(-u_2[i] + a0_r * u_1[i] + 1e-5)
			...
			conf_ave = tf.stack(\
				[self.vars_fuse['conf'],self.vars_fuse['conf']]
			)
			...
			self.input_data = tf.group(\
				I.assign(self.I_in), offset.assign(self.offset_in)
			)
	...
	def process(self):
		...
		I_tmp = np.array(I_cache, np.float32)

		# # get rid of the saturated 
		# flag = np.where(I_tmp > 250)
		# I_tmp[flag] = np.nan

		self.input_dict[self.I_in] = I_tmp		
	...
	def robust_depth(self, old, new, conf):
	...
	def robust_track_Z(self):
		...
		conf = copy.deepcopy(self.results['conf'])
		...
		if self.Z_high.flatten().shape[0] > 0:
			...
			# self.Z_tgt = np.abs(np.median(Z_high_roi))
	...
	def offset_to_a1(self, offset):
		o = np.array([(offset/10000)**k for k in range(len(self.cfg[0]['a1_o']))])
		a1 = np.sum(o*self.cfg[0]['a1_o'])
		return a1
	...
	def a1_to_offset(self, a1):
		...
		coeff = np.flipud(copy.deepcopy(self.cfg[0]['a1_o']))
		coeff[-1] -= a1
		p = np.roots(coeff)
		p = p[np.where(np.imag(p)==0)]
		p = np.real(p)[0]
		return int(p*10000)
...
class Display(threading.Thread):
	...
	def iccv_output(self):	
		...
		# Z_f = self.cfg[0]['a1']
		# flag = np.where(\
		# 		np.abs(np.abs(self.results['Z']) - Z_f) > 0.2*Z_f
		# 	)
		# self.results['Z'][flag] = -1e-10
		# self.results['conf'][flag] = 0
		...
		# self.results['Z'][np.where(self.results['conf']<conf_thre)] = -1e-10
		# # self.results['Zf'] = self.guided(\
		# # 	self.results['I_0'],\
		# # 	self.results['Zf'], \
		# # 	self.results['conf']\
		# # )
		# self.results['Z'] = self.jointBilateral(\
		# 	self.results['I_0'],\
		# 	self.results['Z'], \
		# 	self.results['conf']\
		# )
		...
		# tmp = self.results['Z'][np.where(Z[:,:,2]==1)]
		# tmp1 = Z[:,:,1][np.where(Z[:,:,2]==1)]
		# tmp2 = Z[:,:,0][np.where(Z[:,:,2]==1)]

		# create the image to draw
		Z = self.prep_for_draw_demo(I = Z, message='Depth', rng=KEY_RANGE['raw'])
		# flg1 = (Z[:,:,1]==0)
		# flg2 = (Z[:,:,2]==0)
		# Z[:,:,0][np.where(flg1*flg2)] = 0
		# tmp = self.results['conf'][np.where(flg1*flg2)]
		# pdb.set_trace()
		I = self.prep_for_draw_demo(I = np.abs(self.results['I_0']), message='Input image', rng=KEY_RANGE['raw'])
		self.cache['draw'] = np.concatenate((I,Z), axis=1)
...
def multithreading_test():
	...
	cfg_file = "./opt_results/pyConfLensFlowNetFast_iccv4/"+\
		"final.pickle"
	with open(cfg_file,'rb') as f:
		cfgf_data = pickle.load(f)
	cfgf = cfgf_data['cfg']

	keys = ['szx_sensor','szy_sensor','w_bc','w_bc1','w_bc2']
	for i in range(len(cfgf)):
		for key in keys:
			cfgf[i][key] = cfg[i][key]

	b = PulseCamProcessorTF(cfgf)
	b.start()
