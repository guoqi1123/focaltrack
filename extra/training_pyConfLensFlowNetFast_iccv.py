...
PADDING = 'VALID'
class training_pyConfLensFlowNetFast_iccv(training_focalFlowNet):
	...
	def __init__(self, cfg = None):
		...
		self.cfg.append({
			...
			'outs': 'Z',
			'learn_rate': 0.01,
			'err_func': 'softmax_err',
			'conf_func': 'w3_baseline_conf',
			'fc': \
				np.array(\
					[\
						[[1,1],[1,1],[1,1]],\
			...
			'ac': 1,
			'offsets': [k*1000 for k in range(55,20,-1)],
		})
	...
	def build_graph(self):
		with self.graph.as_default():
			...
			I_init = np.zeros(
				self.resolution[0]+\
				(self.cfg[0]['ft'].shape[2]*len(self.cfg[0]['offsets']),),
				dtype = np.float32
			)
			...
			self.offsets = tf.Variable(self.cfg[0]['offsets'], dtype=tf.float32)
			offsets = tf.Variable(self.cfg[0]['offsets'], dtype=tf.float32)
			...
			for i in range(len(self.cfg)):
				...
				b0.append(tf.Variable(self.cfg[i]['b0'], dtype = tf.float32))
				b1.append(tf.Variable(self.cfg[i]['b1'], dtype = tf.float32))

				a0a1.append(tf.Variable(self.cfg[i]['a0a1'], dtype = tf.float32))				
			...
			self.I_t = []
			self.I_lap = []
			for o in range(len(self.cfg[0]['offsets'])):
				I_t = []
				I_lap = []
				for i in range(len(self.cfg)):
					# Generate the differential images
					I_t.append(dIdt(I_batch[i][:,:,o*2:o*2+2], ft[i]))
					I_lap.append(dIdt(I_lap_batch[i][:,:,o*2:o*2+2], fave[i]))

				self.I_t.append(I_t[0])
				self.I_lap.append(I_lap[0])
				for seq in range(2):
					u_1 = []
					u_2 = []
					u_3 = []
					...
					for i in range(len(self.cfg)):
						# optical parameters
						a1 = 1/(b0[0]*offsets[o]+b1[0])
						a0 = a0a1[i]/a1
					
						# unwindowed version
						u_1.append(I_lap[i])
						u_2.append(I_t[i])
						u_3.append(a0 * a1 * u_1[i])
						u_4.append(-u_2[i] + a0 * u_1[i])
					...
					# record the fused Zs
					self.Z_offsets.append(self.vars_fuse['Z'])
					self.C_offsets.append(self.vars_fuse['conf'])
			...
			self.vars_fuse['Z'] = tf.reduce_sum(
				tf.stack(
					[\
						self.Z_offsets[k]*self.ws[k]\
						for k in range(len(self.Z_offsets))\
					],
				-1),
			-1)
			self.vars_fuse['conf'] = tf.reduce_sum(
				tf.stack(
					[\
						self.C_offsets[k]*self.ws[k]\
						for k in range(len(self.C_offsets))\
					],
				-1),
			-1)
			...
			for i in range(len(self.cfg)):
				...
				for ef in self.cfg[0]['err_func']:
					...
					dLdb0, dLdb1, dLda0a1, dLdZ_0 = tf.gradients(
						self.vars_fuse['loss_func'][ef],\
						[b0[i], b1[i], a0a1[i], Z_0[i]]
					)
					if dLdb0 == None:
						dLdb0 = tf.constant(0,dtype =tf.float32)
					if dLdb1 == None:
						dLdb1 = tf.constant(0,dtype = tf.float32)
					if dLda0a1 == None:
						dLda0a1 = tf.constant(0,dtype =tf.float32)
					if dLdZ_0 == None:
						dLdZ_0 = tf.constant(0,dtype =tf.float32)

					dLdb0 = dLdb0 * self.cfg[i]['db0_ratio']
					dLdb1 = dLdb1 * self.cfg[i]['db1_ratio']
					dLda0a1 = dLda0a1 * self.cfg[i]['da0a1_ratio']
					dLdZ_0= dLdZ_0 * self.cfg[i]['dZ_0_ratio']
					self.der[i]['dLdb0'][ef] = dLdb0
					self.der[i]['dLdb1'][ef] = dLdb1
					self.der[i]['dLda0a1'][ef] = dLda0a1
	...
	def w3_baseline_conf(self):
		...
		if 'w_bc' not in self.__dict__:
			self.w_bc = [] # the weight of baseline confidence for each layer
			self.w_bc1 = []
			self.w_bc2 = []
			for i in range(len(self.cfg)):
				# weights
		...
		for i in range(len(self.cfg)):
			...
			for j in range(self.cfg[0]['ext_f'].shape[0]):
				conf_tmp.append(
					...
					tf.sqrt(\
						self.w_bc[i][j] * self.vars[i]['u_3'][:,:,j]**2 + \
						self.w_bc1[i][j] * self.vars[i]['u_4'][:,:,j]**2 + \
						self.w_bc2[i][j] * self.vars[i]['u_3'][:,:,j]*self.vars[i]['u_4'][:,:,j] + \
						self.vars[i]['u_4'][:,:,j]**4 + \
						1e-10\
					)
				)
			...
			self.vars[i]['w_bc'] = self.w_bc[i]
			self.vars[i]['w_bc1'] = self.w_bc1[i]
			self.vars[i]['w_bc2'] = self.w_bc2[i]
		...
		for i in range(len(self.cfg)):
			for j in range(self.cfg[0]['ext_f'].shape[0]):
				...
				tmp_align = (self.vars_align['u_4'][:,:,idx]**2 + 1e-20)/\
					tf.sqrt(\
						self.w_bc[i][j] * self.vars_align['u_3'][:,:,idx]**2 + \
						self.w_bc1[i][j] * self.vars_align['u_4'][:,:,idx]**2 + \
						self.w_bc2[i][j] * self.vars_align['u_3'][:,:,idx]*self.vars_align['u_4'][:,:,idx] + \
	...
	def softmax_err(self):
		...
		err_offsets = [\
			tf.reduce_mean(\
				tf.abs(\
					self.Z_offsets[k]-self.vars_fuse['Z_gt']\
				)**p\
			)**(1/p) \
			for k in range(len(self.Z_offsets))
		]
	...
	def one_round_loss(self, I, Loc, offsets):
		for i in range(I.shape[0]):
			# input images
			Z_gt = Loc[i,2,int((Loc.shape[2]-1)/2)]
			Z_map_gt = np.ones(I[i,:,:,0].shape) * Z_gt
			self.input_images(I[i,:,:,:], Z_map_gt, offsets[i,:])

			# Update the loss
			self.update_loss()
		return self.print_loss()

	def one_step_training_force(self, I, Loc, offsets, step, min_step, temperature = 0.0):
		...
		self.visual_heatmap(I, Loc, offsets, conf_thre=-np.inf)
		...
		while(step > min_step):
			...
			# run one round
			new_loss = self.one_round_loss(I, Loc, offsets)
			thre = np.exp(
	...
	def visual_heatmap(self, I, Loc, offsets, conf_thre=0.):
		# since the confidence is not necessary max to 1
		# conf_thre is the threshold of the max confidence
		# input images
		Z_gt = Loc[0,2,int((Loc.shape[2]-1)/2)]
		Z_map_gt = np.ones(I[0,:,:,0].shape) * Z_gt
		self.input_images(I[0,:,:,:], Z_map_gt, offsets[0,:])
