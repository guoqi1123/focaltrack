	...
	def run(self):
		...
		while True:
			...
			if chr(c).lower() == 'q' or ending_key == 'q':
				...
				ending_key = 'q'
				break
			# if chr(c).lower() == 's' or ending_key == 's':
...
class PulseCamProcessor(threading.Thread, training_pyConfLensFlowNetFast):
	...
	def run(self):
		...
		while True:
			...
			if chr(c).lower() == 'q' or ending_key == 'q':
				...
				perf = (1.0*self.frames)/(t1-t0)
				print("quitting")
				print("FT avg performance: (gui + compute)", perf, " fps")
				self.final_statistics()
				ending_key = 'q'
				break
			if ending_key == 'r':
	...
	def w3_baseline_conf(self):
		...
		w_bc = tf.concat(0,w_bc)
		w_bc1 = tf.concat(0,w_bc1)
		w_bc2 = tf.concat(0,w_bc2)
	...
	def process(self):
		...
		# run it
		self.image_to_show = ['Z','conf','Zf','conff']
		res_dict = {}
		for k in self.image_to_show:
			res_dict[k] = self.vars_fuse[k]
		self.results = self.session.run(res_dict)

		# try to select the correct sequence
		if self.frames > 2:
			self.results['Z'], self.results['conf'] = self.robust_depth(\
				self.depth_data['Z'],\
				self.results['Z'],\
				self.results['conf'],\
				self.results['Zf'],\
	...
	def robust_depth(self, old, new, conf, newf, conff, conf_thre):
		global ending_key
		# robustly find the correct depth map that is consistent with 
		# the previous result
		# newf = 2*self.cfg[0]['a1'] - new
		err = np.array(\
			[\
				np.mean(np.abs(old - new)[np.where(conf>conf_thre)]), \
				np.mean(np.abs(old - newf)[np.where(conff>conf_thre)]) \
			]\
		)
		depth = [new, newf]
		confs = [conf, conff]
		if ending_key == 's':
			ending_key = 'c'
			return depth[np.argmax(err)],confs[np.argmax(err)]
		else:
	...
	def passive_Z(self):
		self.Z_tgt = np.abs(np.median(self.results['Z'][np.where(self.results['conf'] > 0.999)]))
	...
	def track_Z_pid(self):
		global DEPTH_RANGE
		Z_tmp = self.results['Z'][:,int(self.results['Z'].shape[1]/2):-1]
		conf_tmp = self.results['conf'][:,int(self.results['conf'].shape[1]/2):-1]
		# Z_tmp = self.results['Z']
		# conf_tmp = self.results['conf']
	...
	def run(self):
		...
		while True:
			...
			if chr(c).lower() == 'q' or ending_key == 'q':
				...
				perf = (1.0*self.frames)/(t1-t0)
				print("quitting")
				print("Display avg performance: (gui + compute)", perf, " fps")
				self.final_statistics()
				ending_key = 'q'
				break
			if chr(c) == '\n':
