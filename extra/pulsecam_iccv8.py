...
DEPTH_RANGE_f = [-2,-0.3]#[-1.0,-0.3] # -1.0,-0.5
...
class PulseCamProcessorTF(threading.Thread):
	...
	def build_graph(self):
		with self.graph.as_default():
			...
			for i in range(len(self.cfg)):
				...
				o = offset/10000
				a1f.append(tf.exp(a1f_o[i][0] * o + a1f_o[i][1])+a1f_o[i][2])
	...
	def offset_to_a1_f(self, offset):
		o = offset/10000
		a1f_o = self.cfgf[0]['a1_o']
		a1 = np.exp(a1f_o[0] * o + a1f_o[1])+a1f_o[2]
		return a1

	def a1_to_offset_f(self, a1):
		a1f_o = self.cfgf[0]['a1_o']
		o = (np.log(a1 - a1f_o[2]) - a1f_o[1])/a1f_o[0]
		if a1 < a1f_o[2]:
			pdb.set_trace()
