...
PADDING = 'VALID'
class training_pyConfLensFlowNetFast_ext(training_focalFlowNet):
		...
		with self.graph.as_default():
			...
			for ef in self.cfg[0]['err_func']:
				...
				for i in range(len(grads_and_vars)):
					grads_and_vars[i] = (grads_and_vars[i][0], grads_and_vars[i][1])
	...
	def w3r_baseline_conf(self):
		...
		for i in range(len(self.cfg)):
			for j in range(self.cfg[0]['ext_f'].shape[0]):
				...
				Z = self.vars_align['Z'][:,:,idx]
				cut = tf.sigmoid(-k*(Z-hi[i][j])) * \
					tf.sigmoid(-k*(lo[i][j]-Z))
				# aligned confidence
				conf_align.append(
					tmp_align * cut
				)
	...
	def one_step_training_SGD(self, I, Loc, step=None, min_step=None, temperature=None):
