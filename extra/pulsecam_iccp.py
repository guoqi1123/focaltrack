...
class PulseCamProcessorTF(threading.Thread):
	...
	def scanner_iter(self):
		...
		if self.batch_idx >= self.batch_size:
			...
			if self.offset < self.offset_range[0]:
				...
				self.offset = self.offset_range[1] + self.offset_range[0]
	...
	def offset_to_a1(self, offset):
		return 1/(1e-4*offset-1.9)

	def a1_to_offset(self, a1):
		if a1 == 0:
			pdb.set_trace()
		return int((1/a1 + 1.9)*1e4)
...
class PulseCamProcessorNP(PulseCamProcessorTF):
	def __init__(self, cfg):
		threading.Thread.__init__(self)
		...
		self.prms = {}
		for k in ['w_bc','w_bc1','w_bc2']:
			self.prms[k] = []
			for i in range(len(self.cfg)):
				self.prms[k].append(self.cfg[i][k])
			self.prms[k] = np.concatenate(self.prms[k])
	...
	def computation(self):
		...
		I_t = self.I_cache[:,:,0] - self.I_cache[:,:,1]
		I_ave = self.I_cache[:,:,0] + self.I_cache[:,:,1]

		# compute derivatives for multi channel prediction
		f = [
			self.cfg[0]['ext_f'][i,:,:]\
			for i in range(self.cfg[0]['ext_f'].shape[0])
		]
		I_t_v = np.zeros(I_t.shape+(len(f),))
		I_ave_v = np.zeros(I_lap.shape+(len(f),))

		for i in range(len(f)):
			I_t_v[:,:,i] = convolve2d(I_t, f[i], mode='same', boundary='fill')
			I_ave_v[:,:,i] = convolve2d(I_ave, f[i], mode='same', boundary='fill')


		# image pyramid
		n_layer = 4
		I_t_v = tuple(pyramid_gaussian(I_t_v, max_layer=n_layer-1, downscale=2))
		I_lap_v = tuple(pyramid_laplacian(I_ave_v, max_layer=n_layer-1, downscale=2))

		# resize different channels to the same size
		u_3 = np.zeros(I_t.shape+(len(f)*n_layer,))
		u_4 = np.zeros(I_lap.shape+(len(f)*n_layer,))
		idx = 0
		for i in range(n_layer):
			a0 = self.cfg[i]['a0']
			a1 = self.cfg[i]['a1']

			u_3[:,:,idx:idx+I_t_v[i].shape[2]] = \
				resize(\
					a0*a1*I_lap_v[i], \
					I_t.shape\
				)
			u_4[:,:,idx:idx+I_lap_v[i].shape[2]] = \
				resize(\
					-I_t_v[i] + a0*I_lap_v[i]  + 1e-5,\
					I_lap.shape\
				)
			u_4f[:,:,idx:idx+I_lap_v[i].shape[2]] = \
				resize(\
					I_t_v[i] + a0*I_lap_v[i]  + 1e-5,\
					I_lap.shape\
				)

			idx += I_lap_v[i].shape[2]

		# reshape to make u flat
		u_3 = np.reshape(u_3, (-1, I_t_v.shape[2]))
		u_4 = np.reshape(u_4, (-1, I_t_v.shape[2]))
		u_4f = np.reshape(u_4f, (-1, I_t_v.shape[2]))

		# compute Z and Z_f
		Z_0 = self.cfg[0]['Z_0']
		Z = u_3 / (u_4 + 1e-5) + Z_0
		Zf = u_3 / (u_4f + 1e-5) + Z_0
