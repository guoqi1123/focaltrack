...
DEPTH_RANGE_f = [-4.0,-0.3]#[-1.0,-0.3] # -1.0,-0.5
...
class PulseCamProcessorTF(threading.Thread):
	...
	def build_graph(self):
		with self.graph.as_default():
			...
			self.a1f_in = tf.Variable(1, dtype=tf.float32)
			a1f = tf.Variable(1, dtype=tf.float32)

			quad = tf.Variable(self.quad, dtype=tf.float32)
			...
			for i in range(len(self.cfg)):
				...
				a0a1f.append(tf.constant(self.cfgf[i]['a0a1'], dtype= tf.float32))
				a0f.append(a0a1f[i]/a1f)
				...
				u_3f.append(a0f[i] * a1f * u_1[i])
				u_4f.append(-u_2[i] + a0f[i] * u_1[i] + 1e-5)
			...
			self.input_data = tf.group(\
				I.assign(self.I_in), a1.assign(self.a1_in), a1f.assign(self.a1f_in)
			)
	...
	def process(self):
		...
		self.input_dict[self.a1f_in] = self.cfgf[0]['a1']
	...
	def offset_to_a1_f(self, offset):
		return 1/(self.cfgf[0]['b0']*offset+self.cfgf[0]['b1'])

	def a1_to_offset_f(self, a1):
		if a1 == 0:
			pdb.set_trace()
		return int((1/a1 - self.cfgf[0]['b1'])/self.cfgf[0]['b0'])
...
class Display(threading.Thread):
	...
	def iccv_output(self):	
		...
		self.results['Zf'][np.where(self.results['conf']<conf_thre)] = -1e-10
		# self.results['Zf'] = self.guided(\
		# 	self.results['I_0'],\
		# 	self.results['Zf'], \
		# 	self.results['conf']\
		# )
		self.results['Zf'] = self.jointBilateral(\
			self.results['I_0'],\
			self.results['Zf'], \
			self.results['conf']\
		)
...
def multithreading_test():
	...
	b = PulseCamProcessorTF(cfg[0:-1], cfgf[0:-1])
	b.start()

	time.sleep(1)
	d = Display(cfg[0:-1], cfgf[0:-1])	
	d.start()
