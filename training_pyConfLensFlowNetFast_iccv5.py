...
PADDING = 'VALID'
class training_pyConfLensFlowNetFast_iccv5(training_focalFlowNet):
	...
	def interp_a0(self, x_n, i):
		return interpolate.splev(x_n, self.cfg[int(i)]['a0_rtck']).astype(np.float32)

	def interp_a1(self, x_n, i):
		return interpolate.splev(x_n, self.cfg[int(i)]['a1_rtck']).astype(np.float32)
	...
	def build_graph(self):
		with self.graph.as_default():
			...
			for i in range(len(self.cfg)):
				...
				a0.append(tf.py_func(\
					self.interp_a0, \
					[offset, tf.constant(i, dtype=tf.float32)], \
					tf.float32
				))
				a1.append(tf.py_func(\
					self.interp_a1, \
					[offset, tf.constant(i, dtype=tf.float32)], \
					tf.float32
				))
	...
	def visual_heatmap(self, I, Loc, offset, conf_thre=0.):
		...
		conf_thre = [0.9,0.99,0.999,0.9999]
