...
PADDING = 'VALID'
class training_pyConfLensFlowNetFast_iccv2(training_focalFlowNet):
	...
	def build_graph(self):
		with self.graph.as_default():
			...
			for i in range(len(self.cfg)):
				...
				b0.append(tf.Variable(self.cfg[0]['b0'], dtype = tf.float32))
				b1.append(tf.Variable(self.cfg[0]['b1'], dtype = tf.float32))
				
				a0a1.append(tf.Variable(self.cfg[i]['a0a1'], dtype = tf.float32))
				# optical parameters
				a1.append(1/(b0[0]*offset+b1[0]))
				a0.append(a0a1[i]/a1[i])
				...
				u_1.append(I_lap[i])
				u_2.append(I_t[i])
				u_3.append(a0_r[i] * a1_r[i] * u_1[i])
				u_4.append(-u_2[i] + a0_r[i] * u_1[i])
				u_3f.append(a0_r[i] * a1_r[i] * u_1[i])
				u_4f.append(-u_2[i] + a0_r[i] * u_1[i])
			...
			for i in range(len(self.cfg)):
				...
				for ef in self.cfg[0]['err_func']:
					...
					dLdra0_1, dLdra0_2, dLdra1_1, dLdra1_2 = tf.gradients(
						self.vars_fuse['loss_func'][ef],\
						[ra0_1[i], ra0_2[i], ra1_1[i], ra1_2[i]]
					)
					if dLdra0_1 == None:
