...
for i in range(I_new.shape[0]):
	for j in range(I_new.shape[3]):
		I_new[i,:,:,j] = scipy.misc.imresize(I[i,:,:,j], 1/2)
I = I_new
...
if netName == "pyConfLensFlowNetFast" and training == "pretraining":
...
for i in range(len(ff.cfg)):
	rows_cut = int(ff.cfg[i]['gauss'].shape[0]-1)
	cols_cut = int(ff.cfg[i]['gauss'].shape[1]-1)
	A.append(
		np.empty(
			(
				I.shape[0],
				(cfg[i]['szx_sensor'] - rows_cut) * 
				(cfg[i]['szy_sensor'] - cols_cut)
			)
		)
	)
	b.append(
